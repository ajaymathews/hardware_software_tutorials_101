...
dev_t maj_min_num=MKDEV(101,0);/* 101 is the major number and 0 as minor. if same number exists we can assign with a minor number */
/*now the maj_min_num is a structure member*/

static int __init start_fun(void)
{	
	if(register_chrdev_region(maj_min_num,1, "sample_device")==0)
	{
