...
#define intctrl (*((volatile uint32_t *)0xE000ED04)) //ADRESS OF NTERRUPT CONTROL REGITER(ON ALL CORTEX),WRITING ONE TO 26TH BIT OF REG,INTERRUPT IS REQUESTED

#define thread_nums 3                              //DEFINED 3 AS THEAD_NUMS
#define stack_size 100                             //CONTEXT SIZE
#define bus_clock 10000000
#define plusone 1

uint32_t mills_prescalar,periodic_tick;
...
SysTick->CTRL =0;     												//DISBLES THE SYSTICK  //systick reg can only be included if only, device header is added
SysTick->VAL=0;
SysTick->LOAD = (quanta*mills_prescalar)-1;      //QUANTA IN MILLISECONDS, thus 10*10000
...
void threadyield(void){//TO CALL INTERRUPT WITHOUT DEPENDING ON THE QUANTA[CURRENTLY INTRRUPT IS CALLED AND TASK IS SWITCHED WHEN QUANTA SIZE IS REACHED]
	intctrl = 0x04000000;//triggers systick by setting 26th bitof intcrl 1
}


/*
void schedular_round_robin(void)//for single periodic thread
{
	if(++periodic_tick== msperiod)//tick increase in every 1ms, and if 100ms then runs the task3
	{
		task3();//runs every 100ms
		periodic_tick=0;
	}
		
	
	currentptr=currentptr->nextptr;//this line will be executed at each interrupt
}
*/

void schedular_round_robin(void)//for multiple periodic thread
{
	periodic_tick++;
	periodic_tick&=500;
	if((++periodic_tick%100)== 1)//tick increase in every 1ms, and if 100ms then runs the task3
	{
		(*task3)();//runs every 100ms
	}
		if((++periodic_tick%250)== 1)//tick increase in every 1ms, and if 100ms then runs the task3
	{
			(*task5)();
	}
	currentptr=currentptr->nextptr;//this line will be executed at each interrupt
