...
void threadyield(void){//TO CALL INTERRUPT WITHOUT DEPENDING ON THE QUANTA[CURRENTLY INTRRUPT IS CALLED AND TASK IS SWITCHED WHEN QUANTA SIZE IS REACHED]
	SysTick->VAL=0;//need to set the current value of systick to zero, to start from zero after an interrupt
	intctrl = 0x04000000;//triggers systick by setting 26th bitof intcrl 1
}

void semaphoreinit(int32_t *semaphore,int32_t value)//function to initialize the tocken count of semaphore
{
	*semaphore=value;
}
void signalwait(int32_t *semaphore)//function for each thread to access the tocken of the semaphore
{																									 //thread waits if semaphore count is zero till it is incremented
__disable_irq();
	while(*semaphore<=0)//thread waits in this while condition
	{
		__disable_irq();
		//threadyield(); THIS IS NEEDED I BELIVE, NOT FOUND IN VDEO
		__enable_irq();
	}
	*semaphore -=1;
	__enable_irq();
